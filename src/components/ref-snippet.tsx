...
function renderInline(text: string, key?: string | number): ReactNode {
  ...
  while (i < text.length) {
    ...
    if (text[i] === "[") {
      ...
      if (labelEnd !== -1 && text[labelEnd + 1] === "(") {
        ...
        if (urlEnd !== -1) {
          ...
          parts.push(<a key={partKey++} href={url}>{label}</a>);
          i = urlEnd + 1;
          continue;
        }
      }
    ...
    if (text[i] === "*" && text[i + 1] === "*") {
      ...
      if (end !== -1) {
        flush();
        parts.push(<strong key={partKey++}>{text.slice(i + 2, end)}</strong>);
        i = end + 2;
        continue;
      }
    }
    ...
    if (text[i] === "*" && text[i + 1] !== "*") {
      ...
      if (end !== -1) {
        flush();
        parts.push(<em key={partKey++}>{text.slice(i + 1, end)}</em>);
        i = end + 1;
        continue;
      }
    }
...
function renderBlocks(blocks: MdBlock[]): ReactNode[] {
  ...
  for (const block of blocks) {
    ...
    if (block.type === "h1") {
      ...
      out.push(
        <CodeBlock key={idx++} code={block.body} />
      );
...
export function RefSnippet({ slug }: { slug: string }) {
  ...
  const blocks = parseMd(md.trim());
  const rendered = renderBlocks(blocks);
  return <div className="ref-snippet my-4">{rendered}</div>;
}
